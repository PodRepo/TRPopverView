...
#define TABLEVIEW_PADDING 0.0f
#define TABLEVIEW_TEXT_PADDING 15.f
#define TABLEVIEW_BOTTOM 5.f
#define POP_SPACE 5.0f
...
-(id)initWithPoint:(CGPoint)point titles:(NSArray *)titles images:(NSArray *)images
{
    ...
    if (self) {
        ...
        _popStyle = PopoverViewStyleBlack;
...
-(CGRect)getViewFrame
{
    ...
    if ([self.titleArray count] == [self.imageArray count]) {
        frame.size.width = 2*TABLEVIEW_PADDING + 25 + 10 + frame.size.width + TABLEVIEW_TEXT_PADDING;
    }else{
        frame.size.width = 2*TABLEVIEW_PADDING + frame.size.width + TABLEVIEW_TEXT_PADDING;
    }
    ...
    //左间隔最小5x
    if (frame.origin.x < POP_SPACE) {
        frame.origin.x = POP_SPACE;
    }
    //右间隔最小5x
    CGFloat sw = [[UIScreen mainScreen] bounds].size.width;
    if ((frame.origin.x + frame.size.width) > sw - POP_SPACE) {
        frame.origin.x = sw - POP_SPACE - frame.size.width;
    }
    
    return frame;
}

-(void)setPopStyle:(PopoverViewStyle)popStyle{
    _popStyle = popStyle;
    if (self.popStyle == PopoverViewStyleBlack) {
        self.borderColor = RGB(0x46, 0x4A, 0x4F);
        _tableView.separatorColor = RGB(0x66, 0x6B, 0x70);
    }else if(self.popStyle == PopoverViewStyleWhite) {
...
-(UITableView *)tableView
{
    ...
    rect.origin.x = TABLEVIEW_PADDING;
    rect.origin.y = kArrowHeight;
    rect.size.width -= TABLEVIEW_PADDING * 2;
    rect.size.height -= (kArrowHeight + TABLEVIEW_BOTTOM);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) {
        NSBundle *bundle = [NSBundle bundleForClass:[PopoverCell class]];
        NSArray *cells = [bundle loadNibNamed:@"PopoverCell" owner:nil options:nil];
    ...
    if (self.popStyle == PopoverViewStyleBlack) {
        cell.backgroundColor = RGB(0x46, 0x4A, 0x4F);
    ...
    if ([_imageArray count] == [_titleArray count]) {
        ...
        cell.actionImage.image = [UIImage imageNamed:[_imageArray objectAtIndex:indexPath.row]];
        cell.actionLableSpace.constant = 35.0f;
    }else{
        cell.actionLableSpace.constant = 10.0f;
    }
    
    cell.actionLabel.text = [_titleArray objectAtIndex:indexPath.row];
    cell.actionLabel.textAlignment = NSTextAlignmentCenter;
    cell.actionLabel.font = TITLE_FONT;
    
    
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 7.0) {
        cell.separatorInset = UIEdgeInsetsMake(0, 10.0, 0, TABLEVIEW_TEXT_PADDING);
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    }else if (self.popStyle == PopoverViewStyleBlack) {
        [self.borderColor setFill];
    }
    [popoverPath fill];
